...
# V_es = -build_periodic_potential(V_depth_es, k_lattice_actual, x)
# V_es = build_periodic_potential(V_depth_gs, k_lattice_actual, x) # no excited state potential. what what happes?
V_es = V_gs # not non-magic

# import matplotlib.pyplot as plt
# # plt.plot(x / lattice_wavelength_natural, V_gs, label='V_gs')
# # plt.show()
# # print(np.max(x))
# # print(np.max(x)/lattice_wavelength_natural)
# # print(k_lattice_actual)
# print(lattice_wavelength_natural/ dx)
# print(dx * x_scale) # 1.32 nm dx
# print(k_lattice_natural / dp)
# exit(0)
# plt.plot(x, V_es, label='V_es')
...
print(f"  ν_trap_gs           =     {nu_trap_gs * Gamma_imaging_actual * 1e-3 * 1/(2*np.pi):.3f} (kHz)")
print(f"  ν_trap_es           =     {nu_trap_es * Gamma_imaging_actual * 1e-3 * 1/(2*np.pi):.3f} (kHz)")
...
def run_trajectory(iter_idx):
    ...
    # init_temp = 20 # in uK
    stdevp0 = np.sqrt(init_temp * 0.2)* p0 / (vfactor**0.25) # calibrated so as to maintain initial state temperature of 20uK

    psi_tls_init = g
    psi_init, psi_init_x = build_displaced_vac_state(
    ...
    #     tempinit += (energies)*V_depth_kelvin*1e6
    # print(f"Average initial temperature over {nsamples} samples: {tempinit/nsamples:.1f} uK")
    # exit(0)
    # print(f"Initial temperature: {(energies)*V_depth_kelvin*1e6:.1f} uK")

    # import matplotlib.pyplot as plt
    # plt.plot(x / lattice_wavelength_natural, (np.abs(psi_init_x) ** 2)[1, :], label=r"$|\psi|^2$")
    # plt.show()
    # exit(0)
    # print((hbar/stdevp0) / lattice_wavelength_natural)
    # exit(0)
    # import matplotlib.pyplot as plt
    # fig, ax1 = plt.subplots()

    # x_plot = x / lattice_wavelength_natural
    # l1, = ax1.plot(x_plot, (np.abs(psi_init_x) ** 2)[1, :], color="tab:blue", label=r"$|\psi|^2$")
    # ax1.set_xlabel(r"$x / \lambda_{\mathrm{lattice}}$")
    # ax1.set_ylabel(r"$|\psi|^2$", color="tab:blue")
    # ax1.tick_params(axis="y", labelcolor="tab:blue")

    # ax2 = ax1.twinx()
    # l2, = ax2.plot(x_plot, V_gs, color="tab:red", label="V_gs")
    # ax2.set_ylabel("V_gs", color="tab:red")
    ...
    del psip_store, jumps, norms_arr_img, x_op_t_n, xsq_op_t_n, p_op_t_n, psq_op_t_n, Hxg_op_t_n, Hxe_op_t_n, Hp_op_t_n, psi_final
...
os.makedirs('lattice1D', exist_ok=True)
output_dir = f"lattice1D/corrected-{vfactor:.1f}mK-Vg"
